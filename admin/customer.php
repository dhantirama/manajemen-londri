...
include '../koneksi.php';
$siswa = mysqli_query($koneksi, "SELECT jurusan.jurusan, gelombang.gelombang, customer.* FROM customer LEFT JOIN gelombang ON gelombang.id = customer.id_gelombang LEFT JOIN jurusan ON jurusan.id = customer.id_jurusan WHERE customer.status = 1 ORDER BY id DESC ");

if (isset($_GET['edit'])) {
    $id = $_GET['edit'];
    $update = mysqli_query($koneksi, "UPDATE customer SET status = 2 WHERE id ='$id'");
